###############################################################################
# Local variables for cloudbuild pipellines
###############################################################################

locals {
  gar_name                    = "cloudbuild"
  cloudbuild_image            = "${var.location}-docker.pkg.dev/${module.resources.projects[local.environment.project_pipelines].project_id}/${local.gar_name}/terraform@sha256"
...
module "pipeline_kms_key" {
  ...
  name          = module.resources.projects[local.environment.project_pipelines].project_id
  key_ring_name = module.resources.projects[local.environment.project_pipelines].project_id
...
module "artifact_registry" {
  ...
  depends_on = [module.resources.projects, module.pipeline_kms_key.encrypters, module.pipeline_kms_key.decrypters]
}
...
resource "null_resource" "cloudbuild_terraform_builder" {
  triggers = {
    project_id                  = module.resources.projects[local.environment.project_pipelines].project_id
  ...
  provisioner "local-exec" {
    command = <<EOT
    gcloud builds submit ${path.module}/cloudbuild_builder/ --project ${module.resources.projects[local.environment.project_pipelines].project_id} --config=${path.module}/cloudbuild_builder/cloudbuild.yaml --substitutions=_GCLOUD_VERSION=${local.gcloud_version},_TERRAFORM_VERSION=${local.terraform_version},_TERRAFORM_VERSION_SHA256SUM=${local.terraform_version_sha256sum},_REGION=${module.artifact_registry.location},_REPOSITORY=${local.gar_name}
  EOT
  }
...
resource "google_service_account_iam_member" "sa_cloudbuild_token_creator" {
  ...
  service_account_id = module.resources.service_accounts[each.key].name
  role               = "roles/iam.serviceAccountTokenCreator"
  member             = "serviceAccount:service-${module.resources.projects[local.environment.project_pipelines].number}@gcp-sa-cloudbuild.iam.gserviceaccount.com"
}
...
resource "google_cloudbuild_trigger" "plan-trigger" {
  ...
  service_account = module.resources.service_accounts[each.value.service_account].id
