...
resource "google_service_account_iam_member" "wif-sa-bind" {
  ...
  member             = "principalSet://iam.googleapis.com/projects/${module.projects[var.seed_project_name].number}/locations/global/workloadIdentityPools/${google_iam_workload_identity_pool.github[each.key].workload_identity_pool_id}/attribute.repository/${each.value.repository}"
}